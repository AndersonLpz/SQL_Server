...
BEGIN
	...
	SELECT @characters = '0123456789abcdefghijklmnopqrstuvwxyz',@Nr_Sequencia = 0,@Id_Objeto_Pai = 0;
	...
	WHILE (1 = 1)BEGIN
		...
		WHILE (( PATINDEX('%[A-Za-z0-9@+.e]%', @Contents COLLATE SQL_Latin1_General_CP850_BIN) ) <> 0)BEGIN
			...
			IF (@Type = 'Object')BEGIN
				...
				SELECT @Nr_Sequencia = 0, @end = CHARINDEX(':', ' ' + @Contents)
			...
			END	ELSE
				SELECT @Ds_Nome = NULL, @Nr_Sequencia = @Nr_Sequencia + 1
			...
			IF (SUBSTRING(@value, 1, 7) = '@object') BEGIN
