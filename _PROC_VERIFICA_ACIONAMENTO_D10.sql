...
IF OBJECT_ID('dbo._PROC_VERIFICA_ACIONAMENTO_D10', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].[_PROC_VERIFICA_ACIONAMENTO_D10]
GO
...
CREATE PROCEDURE _PROC_VERIFICA_ACIONAMENTO_D10
AS
	/***************************************************************************************************************************************************************************************
	*	AUTOR..........: ANDERSON LOPEZ
	*	DATA...........: 10/08/2022..........ANDERSON LOPEZ	
	*	DATA...........: 06/10/2022..........ANDERSON LOPEZ	
	...
	***************************************************************************************************************************************************************************************/
	...
	/***************************************************************************************************************************************************************************************/
	IF Object_ID('tempDB..##TB_ACIONAMETO','U') IS NOT NULL				DROP TABLE ##TB_ACIONAMETO;	
	/**************************************************************************************************************************************************************************************/	
...
BEGIN 	
	...
	***************************************************************************************************************************************************************************************/
		...
		INTO
			#TB_CEDENTE_AUX
		FROM 
			[Homolog].[dbo].[TB_CEDENTE]										DB_CEDE
			JOIN [Homolog].[dbo].[TB_PRODUTO]									DB_PROD ON DB_CEDE.ID_CEDENTE = DB_PROD.ID_CEDENTE 
			JOIN [Homolog].[dbo].[TB_PRODUTO_DETALHE]							DB_PRDE ON DB_PROD.ID_PRODUTO = DB_PRDE.ID_PRODUTO
		WHERE
			DB_CEDE.ID_CEDENTE NOT IN(1, 13, 55, 68, 48, 60)
	...
	***************************************************************************************************************************************************************************************/	
		...
		***************************************************************************************************************************************************************************************/
			IF ((SELECT COUNT(CEDENTE_GRUPO_NS) FROM ##TB_ACIONAMETO WHERE CEDENTE_GRUPO_NS = @CEDENTE_GRUPO AND CEDENTE_NM_FANTASIA = '_CEDENTE') = 0)BEGIN
				INSERT INTO ##TB_ACIONAMETO
				SELECT
		...
		***************************************************************************************************************************************************************************************/
			...
			FROM
				[Homolog].[dbo].[TB_CEDENTE]										DB_CEDE
				JOIN [Homolog].[dbo].[TB_CLIENTE]									DB_CLIE ON DB_CEDE.ID_CEDENTE = DB_CLIE.ID_CEDENTE
			...
			WHERE
				...
				AND 1 = (
					CASE
						WHEN (SELECT COUNT(DB_CLAC.ID_ACAO) FROM [Homolog].[dbo].[TB_CLIENTE_ACAO] DB_CLAC WHERE DB_CLAC.ID_CLIENTE = DB_CLIE.ID_CLIENTE) = 0 THEN 1
						ELSE 0
						END
				);
	...
	*	EXPORTAR PARA .CSV
	***************************************************************************************************************************************************************************************/
		WHILE(EXISTS(SELECT TOP 1 CEDENTE_GRUPO_NS FROM ##TB_ACIONAMETO WHERE FLAG = 0 AND CEDENTE_NM_FANTASIA != '_CEDENTE'))BEGIN
			SELECT TOP 1 @CEDENTE_GRUPO = CEDENTE_GRUPO_NS, @CEDENTE_GRUPO_NM = SEGMENTO FROM ##TB_ACIONAMETO WHERE FLAG = 0 AND CEDENTE_NM_FANTASIA != '_CEDENTE';

			IF((SELECT COUNT(CEDENTE_GRUPO_NS) FROM ##TB_ACIONAMETO WHERE CEDENTE_GRUPO_NS = @CEDENTE_GRUPO) > 1) BEGIN		
			
				SET @NM_ARQUIVO = CONVERT(CHAR(8), GETDATE(),112) + '_D10_' + @CEDENTE_GRUPO_NM;
				...
				SET @SQL_CONSULTA += '	##TB_ACIONAMETO ';
			...
			END;			
			UPDATE ##TB_ACIONAMETO SET FLAG = 1 WHERE  CEDENTE_GRUPO_NS = @CEDENTE_GRUPO;
		END;
	/***************************************************************************************************************************************************************************************
	*	ENVIO EMAIL
	***************************************************************************************************************************************************************************************/	
		...
		WHILE(EXISTS(SELECT TOP 1 CEDENTE_GRUPO_NS FROM ##TB_ACIONAMETO WHERE FLAG = 1 AND CEDENTE_NM_FANTASIA != '_CEDENTE'))BEGIN
			SELECT TOP 1 @CEDENTE_GRUPO = CEDENTE_GRUPO_NS, @CEDENTE_GRUPO_NM = SEGMENTO FROM ##TB_ACIONAMETO WHERE FLAG = 1 AND CEDENTE_NM_FANTASIA != '_CEDENTE';
			SET @EMAIL_ENVIO_NS = 0;
		/***************************************************************************************************************************************************************************************
		*	RESETA OS VALORES DAS VARIAVEIS
		***************************************************************************************************************************************************************************************/
			SET @EMAIL_ENVIO_RECIPIENTES = '';			
			SET @EMAIL_SUBJECT = 'Dividas D+10 sem acionamento';
			SET @NM_ARQUIVO = CONVERT(CHAR(8), GETDATE(),112) + '_D10_' + @CEDENTE_GRUPO_NM;
			...
			IF(@EMAIL_ENVIO_NS = 1)BEGIN
				...
				SET @BODY_ENVIO_HTML += N'<p>Segue a relação das dividas que estão em nossa base onde estao a 10 ou mais sem acionamento.</p>';
				...
				SELECT
					...
					, @EMAIL_ENVIO_FILE_ATT;
			END;
			DELETE FROM ##TB_ACIONAMETO WHERE  CEDENTE_GRUPO_NS = @CEDENTE_GRUPO;
		END;
